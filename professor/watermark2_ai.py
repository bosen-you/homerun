"""
實作更深入和廣泛的聲音浮水印應用。我們將增加一些功能，
例如多層水印嵌入、不同的嵌入方法，以及更強的抗干擾能力。
使用 TensorFlow 來訓練一個簡單的自編碼器（Autoencoder）模型，用於嵌入和提取水印。
"""
import numpy as np
...
# 保存模型
autoencoder.save('audio_autoencoder.h5')
...
# 加載模型
autoencoder = load_model('audio_autoencoder.h5')
...
wavfile.write('output_with_watermark.wav', rate, encoded_audio)
print(f'嵌入的水印: {watermark}',', 檔名:output_with_watermark.wav')
